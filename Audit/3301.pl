...
sub compare_acl_policy_3301 {
	...
	my $redis_handle = connect_redis_3301;
...
sub include_src_addr_3301 {
	...
	if($scard_set1 != $scard_set2){
		if ($scard_set1 == $scard_interset || $scard_set2 == $scard_interset) {
	    	#print "acl_id $index1 $index2 src_addr include\n";
     		$redis_handle->quit;
     		return 0;
   		}
    }
...
#				#print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3301($1);
								my $end = ip2num_3301($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3301($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3301($ip) - 1);
								push @break_num, (ip2num_3301($ip) + 1);
							}

							push @break_num, ip2num_3301($1);
							push @break_num, ip2num_3301($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3301($_));
								}
							}
						} else {
							my @break_num;
...
#			#print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_dst_addr_3301($array[$j], $array[$k]);
			my $ret2 = compare_srv_3301($array[$j], $array[$k]);
			my $ret3 = compare_acl_policy_3301($array[$j], $array[$k]);
			my $ret4 = include_src_addr_3301($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 != 0 and $ret4 == 0 and $ret5 ==1) {
				#print "\n*****************************\n";
				#print "acl_id $array[$j], $array[$k] same in dst_addr srv,different in acl_policy but src_addr is included\n";
				my $sth_src = count_src($task_id,$array[$j],$array[$k]);
				pass $task_id,3301,'fail',"访问控制策略$array[$j]与$array[$k]之间存在源地址全部冲突. "."$sth_src";
				$sign_3301=1;
			}
			$k++;
		}
	...
	if($sign_3301 == 0){pass $task_id,3301,'pass',"访问控制策略之间不存在源地址全部冲突";}

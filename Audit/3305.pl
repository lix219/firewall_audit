...
#				#print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3305($1);
								my $end = ip2num_3305($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3305($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3305($ip) - 1);
								push @break_num, (ip2num_3305($ip) + 1);
							}

							push @break_num, ip2num_3305($1);
							push @break_num, ip2num_3305($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3305($_));
								}
							}
						} else {
							my @break_num;
...
#			#print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_src_addr_3305($array[$j], $array[$k]);
			my $ret2 = inter_dst_addr_3305($array[$j], $array[$k]);
			my $ret3 = compare_srv_3305($array[$j], $array[$k]);
			my $ret4 = compare_acl_policy_3305($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 == -1 and $ret5 ==1) {
				#print "\n*****************************\n";
				#print "acl_id $array[$j], $array[$k] same in src_addr srv, different in acl_policy but have interset in dst_addr\n";
				my $sth_dst = count_src($task_id,$array[$j],$array[$k]);
				pass $task_id,3305,'fail',"访问控制策略$array[$j]与$array[$k]之间存在目的地址部分冲突. "."$sth_dst";
				$sign_3305 = 1;
			}
	...
	if($sign_3305 == 0){pass $task_id,3305,'pass',"访问控制策略之间不存在目的地址部分冲突";}

...
sub compare_acl_policy_3204 {
	...
	my $redis_handle = connect_redis_3204;
...
sub inter_src_addr_3204 {
	...
	if ($scard_interset > 0 and $scard_interset < $scard_set1 and $scard_interset < $scard_set2) {
		print "acl_id $index1 and $index2 have interset in src_addr\n";
		$redis_handle->quit;
		return 0;
	}
...
#				print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3204($1);
								my $end = ip2num_3204($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3204($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3204($ip) - 1);
								push @break_num, (ip2num_3204($ip) + 1);
							}

							push @break_num, ip2num_3204($1);
							push @break_num, ip2num_3204($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3204($_));
								}
							}
						} else {
							my @break_num;
...
#			print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_dst_addr_3204($array[$j], $array[$k]);
			my $ret2 = compare_srv_3204($array[$j], $array[$k]);
			my $ret3 = compare_acl_policy_3204($array[$j], $array[$k]);
			my $ret4 = inter_src_addr_3204($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 == 0) {
				print "\n*****************************\n";
				print "acl_id $array[$j], $array[$k] have interset in src_addr\n";
				my $sth_src = count_src($task_id,$array[$j],$array[$k]);
				pass $task_id, 3204, 'fail', "访问控制策略$array[$j]与$array[$k]之间存在源地址部分冗余. "."$sth_src";
				record 3204, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间存在源地址部分冗余";
			} else {
#				pass $task_id, 3204, 'pass', "访问控制策略 $array[$j] 与 $array[$k] 之间不存在源地址部分冗余";
				record 3204, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间不存在源地址部分冗余";
			}
			$k++;
		}
		shift @array;

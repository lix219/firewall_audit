...
sub compare_acl_policy_3300 {
	...
	my $redis_handle = connect_redis_3300;
...
#				#print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3300($1);
								my $end = ip2num_3300($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3300($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3300($ip) - 1);
								push @break_num, (ip2num_3300($ip) + 1);
							}

							push @break_num, ip2num_3300($1);
							push @break_num, ip2num_3300($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3300($_));
								}
							}
						} else {
							my @break_num;
...
#			#print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_src_addr_3300($array[$j], $array[$k]);
			my $ret2 = compare_dst_addr_3300($array[$j], $array[$k]);
			my $ret3 = compare_srv_3300($array[$j], $array[$k]);
			my $ret4 = compare_acl_policy_3300($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id,$array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 != 0 and $ret5 ==1) {
				#print "\n******************************\n";
				#print "acl_id $array[$j], $array[$k] completely different\n";
				pass $task_id,3300,'fail',"访问控制策略$array[$j]与$array[$k]作用相反";
				$sign_3300=1;
			}
	...
	if($sign_3300 == 0){pass $task_id,3300,'pass',"不存在作用相反的访问控制策略";}

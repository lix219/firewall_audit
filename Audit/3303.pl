...
sub compare_acl_policy_3303 {
	...
	my $redis_handle = connect_redis_3303;
...
sub include_srv_3303 {
	...
	if ($sign == 1) {
		foreach (keys %srv_hash2) {
			if (! exists($srv_hash1{$_})) {
				#print "acl_id $index2 is not included in $index1 srv\n";
				$redis_handle->quit;
				return -1;
			}
		}
		...
		foreach (keys %srv_hash1) {
			if (! exists($srv_hash2{$_})) {
				#print "acl_id $index1 is not included in $index2 srv\n";
				$redis_handle->quit;
				return -1;
			}
		}
	...
	if ($sign == 0) {
		#print "acl_id $index1 is included in $index2 srv\n";
	} else {
		#print "acl_id $index2 is included in $index1 srv\n";
	}
...
#				#print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3303($1);
								my $end = ip2num_3303($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3303($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3303($ip) - 1);
								push @break_num, (ip2num_3303($ip) + 1);
							}

							push @break_num, ip2num_3303($1);
							push @break_num, ip2num_3303($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3303($_));
								}
							}
						} else {
							my @break_num;
...
#			#print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_src_addr_3303($array[$j], $array[$k]);
			my $ret2 = compare_dst_addr_3303($array[$j], $array[$k]);
			my $ret3 = include_srv_3303($array[$j], $array[$k]);
			my $ret4 = compare_acl_policy_3303($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 == -1 and $ret5 ==1) {
				#print "\n***********************************\n";
				#print "acl_id $array[$j], $array[$k] same in src_addr dst_addr, different in acl_policy but srv is included\n";
				my $sth_ser = count_src($task_id,$array[$j],$array[$k]);
				pass $task_id,3303,'fail',"访问控制策略$array[$j]与$array[$k]之间存在服务全部冲突. "."$sth_ser";
				$sign_3303=1;
			}
	...
	if($sign_3303 == 0){pass $task_id,3303,'pass',"访问控制策略之间不存在服务全部冲突";}

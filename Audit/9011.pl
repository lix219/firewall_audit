...
print "Execute 9011\n";
	# 获取4006对应的警告内容
	my $warning_sql = "SELECT description FROM audit_items WHERE item_id=9011";
	...
	while (my $row = $result->fetchrow_array) {
		...
		foreach (@arr) {
			if ($_ =~ /TELNET/i) {
				my $insert_sql = "INSERT INTO audit_results VALUES ($task_id, null, 9011, 'fail', \'$warning\', NOW())";
				print "$insert_sql\n";
		...
		if ($sign == 0)	{
			my $insert_sql = "INSERT INTO audit_results VALUES ($task_id, null, 9011, 'pass', '敏感端口1521未向外对互联网开放', NOW())";
			print "$insert_sql\n";
			my $insert_result = $dbh->prepare($insert_sql);
			$insert_result->execute() or die "Can't execute:  $dbh->errstr";
		}

...
use Firewall::Audit qw /acl_schedule/;
...
sub compare_acl_policy_3200 {
	...
	my $redis_handle = connect_redis_3200;
			...
			if ($i == 0) {
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3200($1);
								my $end = ip2num_3200($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3200($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3200($ip) - 1);
								push @break_num, (ip2num_3200($ip) + 1);
							}

							push @break_num, ip2num_3200($1);
							push @break_num, ip2num_3200($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3200($_));
								}
							}
						} else {
							my @break_num;
...
#				print "$acl_policy_set $item\t";
	...
	while ($j < $#array) {
		...
		while ($k <= $#array) {
			...
			my $ret1 = compare_src_addr_3200($array[$j], $array[$k]);
			my $ret2 = compare_dst_addr_3200($array[$j], $array[$k]);
			my $ret3 = compare_srv_3200($array[$j], $array[$k]);
			my $ret4 = compare_acl_policy_3200($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id,$array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 == 0 and $ret5 == 1) {
				print "\n******************************\n";
				print "acl_id $array[$j], $array[$k] all the same\n";
				pass $task_id,3200,'fail',"访问控制策略$array[$j]与$array[$k]出现策略相同";
				record 3200,"TASK[$task_id] 访问控制策略 $array[$j]与$array[$k]出现策略相同";
			} else {
#				pass $task_id,3200,'pass',"访问控制策略 $array[$j]与$array[$k]未出现策略相同";
				record 3200, "TASK[$task_id] 访问控制策略 $array[$j]与$array[$k]未出现策略相同";
			}

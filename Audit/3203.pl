...
#				print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3203($1);
								my $end = ip2num_3203($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3203($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3203($ip) - 1);
								push @break_num, (ip2num_3203($ip) + 1);
							}

							push @break_num, ip2num_3203($1);
							push @break_num, ip2num_3203($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3203($_));
								}
							}
						} else {
							my @break_num;
...
#			print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_src_addr_3203($array[$j], $array[$k]);
			my $ret2 = compare_dst_addr_3203($array[$j], $array[$k]);
			my $ret3 = include_srv_3203($array[$j], $array[$k]);
			my $ret4 = compare_acl_policy_3203($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 == 0 and $ret5 ==1) {
				print "\n***********************************\n";
				print "acl_id $array[$j], $array[$k] all the same in src_addr dst_addr acl_policy but srv is included\n";
				my $sth_ser = count_ser($task_id,$array[$j],$array[$k]);
				pass $task_id, 3203, 'fail', "访问控制策略$array[$j]与$array[$k]之间存在服务全部冗余. "."$sth_ser";
				record 3203, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间存在服务全部冗余";
			} else {
#				pass $task_id, 3203, 'pass', "访问控制策略 $array[$j] 与 $array[$k] 之间不存在服务全部冗余";
				record 3203, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间不存在服务全部冗余";
			}

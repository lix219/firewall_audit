...
print "Execute 9006\n";
	# 获取9006对应的警告内容
	my $warning_sql = "SELECT description FROM audit_items WHERE item_id=9006";
	...
	while(my $row = $result->fetchrow_array)
	{
		...
		foreach (@arr) {
			if ($_ =~ /any/i) {
				my $insert_sql = "INSERT INTO audit_results VALUES ($task_id, null, 9006, 'fail', \'$warning\', NOW())";
				print "$_\t$insert_sql\n";
		...
		if ($sign == 0)	{
			my $insert_sql = "INSERT INTO audit_results VALUES ($task_id, null, 9006, 'pass', '目的端口未出现any或未设置情况', NOW())";
			print "$insert_sql\n";
			my $insert_result = $dbh->prepare($insert_sql);
			$insert_result->execute() or die "Can't execute:  $dbh->errstr";
		}

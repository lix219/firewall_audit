...
use Firewall::Audit qw /acl_schedule count_dst connect_redis/;
...
sub compare_src_addr_3202 {
	...
	if ($scard_interset == $scard_set1 and $scard_set1 == $scard_set2) {
		print "acl_id $index1 and $index2 are totally same in src_addr\n";
		$redis_handle->quit;
		return 0;
	}
...
sub include_dst_addr_3202 {
	...
	if ($scard_interset == $scard_set1 and $scard_set1 < $scard_set2) {
		print "acl_id $index1 is included in $index2 dst_addr\n";
		$redis_handle->quit;
		return 0;
	} elsif ($scard_interset == $scard_set2 and $scard_set2 < $scard_set1) {
		$redis_handle->quit;
		print "acl_id $index2 is included in $index1 dst_addr\n";
		return 0;
	}
...
#				print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3202($1);
								my $end = ip2num_3202($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3202($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3202($ip) - 1);
								push @break_num, (ip2num_3202($ip) + 1);
							}

							push @break_num, ip2num_3202($1);
							push @break_num, ip2num_3202($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3202($_));
								}
							}
						} else {
							my @break_num;
...
#			print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_src_addr_3202($array[$j], $array[$k]);
			my $ret2 = include_dst_addr_3202($array[$j], $array[$k]);
			my $ret3 = compare_srv_3202($array[$j], $array[$k]);
			my $ret4 = compare_acl_policy_3202($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 == 0 and $ret5 ==1) {
				print "\n***************************************\n";
				print "acl_id $array[$j], $array[$k] all the same in src_addr srv acl_policy but dst_addr is included\n";
				my $sth_dst = count_dst($task_id,$array[$j],$array[$k]);
#				my $message = "访问控制策略 $array[$j] 与 $array[$k] 之间存在目的地址全部冗余"."$sth_dst";
#				pass $task_id, 3202, 'fail', "访问控制策略 $array[$j] 与 $array[$k] 之间存在目的地址全部冗余 ;"."$sth_dst";
				pass $task_id, 3202, 'fail', "访问控制策略 $array[$j]与$array[$k]之间存在目的地址全部冗余. "."$sth_dst";
#				print "============$message\n"
#				pass task_id, 3202, 'fail', "$message";
				record 3202, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间存在目的地址全部冗余";
			} else {
#				pass $task_id, 3202, 'pass', "访问控制策略 $array[$j] 与 $array[$k] 之间不存在目的地址全部冗余";
				record 3202, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间不存在目的地址全部冗余";
			}

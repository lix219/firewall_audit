...
sub compare_acl_policy_3304 {
	...
	my $redis_handle = connect_redis_3304;
...
#				#print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3304($1);
								my $end = ip2num_3304($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3304($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3304($ip) - 1);
								push @break_num, (ip2num_3304($ip) + 1);
							}

							push @break_num, ip2num_3304($1);
							push @break_num, ip2num_3304($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3304($_));
								}
							}
						} else {
							my @break_num;
...
#			#print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_dst_addr_3304($array[$j], $array[$k]);
			my $ret2 = compare_srv_3304($array[$j], $array[$k]);
			my $ret3 = compare_acl_policy_3304($array[$j], $array[$k]);
			my $ret4 = inter_src_addr_3304($array[$j], $array[$k]);
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);
			#print "AAA$ret1\t$ret2\t$ret3\t$ret4\n";
			if ($ret1 == 0 and $ret2 == 0 and $ret3 != 0 and $ret4 == 0 and $ret5 ==1) {
				#print "\n*****************************\n";
				#print "acl_id $array[$j], $array[$k] same in dst_addr srv, different in acl_policy but have interset in src_addr\n";
				my $sth_src = count_src($task_id,$array[$j],$array[$k]);
				pass $task_id,3304,'fail',"访问控制策略$array[$j]与$array[$k]之间存在源地址部分冲突. "."$sth_src";
				$sign_3304=1;
			}
			$k++;
		}
	...
	if($sign_3304 == 0){pass $task_id,3304,'pass',"访问控制策略之间不存在源地址部分冲突";}

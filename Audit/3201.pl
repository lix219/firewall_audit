...
use Firewall::Audit qw /acl_schedule count_src/;
...
sub compare_acl_policy_3201 {
	...
	my $redis_handle = connect_redis_3201;
...
sub include_src_addr_3201 {
	...
	if ($scard_interset == $scard_set1 and $scard_set1 < $scard_set2) {
		print "acl_id $index1 is included in $index2 src_addr\n";
		$redis_handle->quit;
		return 0;
	} elsif ($scard_interset == $scard_set2 and $scard_set2 < $scard_set1) {
		$redis_handle->quit;
		print "acl_id $index2 is included in $index1 src_addr\n";
		return 0;
	}
...
sub compare_dst_addr_3201 {
	...
	if ($scard_interset == $scard_set1 and $scard_set1 == $scard_set2) {
		print "acl_id $index1 and $index2 are totally same in dst_addr\n";
		$redis_handle->quit;
		return 0;
	}
...
#				print "$src_addr_set $item\t";
				...
				foreach my $temp (@arr) {
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								my $begin = ip2num_3201($1);
								my $end = ip2num_3201($2);

								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($src_addr_set, num2ip_3201($_));
								}
							}
						} else {
							my @break_num;
							...
							foreach my $ip (@break_ip) {
								push @break_num, (ip2num_3201($ip) - 1);
								push @break_num, (ip2num_3201($ip) + 1);
							}

							push @break_num, ip2num_3201($1);
							push @break_num, ip2num_3201($2);
					...
					if (index($temp, '-') == -1) {
						...
						if (index($temp, '<') == -1) {
							if ($temp =~ /(.*)-(.*)/) {
								...
								foreach ($begin..$end) {
									my $ret = $redis_handle->sadd($dst_addr_set, num2ip_3201($_));
								}
							}
						} else {
							my @break_num;
...
#			print $array[$j], "\t", $array[$k], "\n";
			my $ret1 = compare_dst_addr_3201($array[$j], $array[$k]);
			my $ret2 = compare_srv_3201($array[$j], $array[$k]);
			my $ret3 = compare_acl_policy_3201($array[$j], $array[$k]);
			my $ret4 = include_src_addr_3201($array[$j], $array[$k]);
			#add acl policy effective time
			my $ret5 = acl_schedule($task_id, $array[$j], $array[$k]);	#1 if two policies have intersection about time, else get 0
			if ($ret1 == 0 and $ret2 == 0 and $ret3 == 0 and $ret4 == 0 and $ret5 == 1) {
				print "\n*****************************\n";
				print "acl_id $array[$j], $array[$k] all the same in dst_addr srv acl_policy but src_addr is included\n";
				my $sth_src = count_src($task_id,$array[$j],$array[$k]);
				#my $sth_ser = count_ser($task_id,$array[$j],$array[$k]);
				pass $task_id, 3201, 'fail', "访问控制策略$array[$j]与$array[$k]之间存在源地址全部冗余. "."$sth_src";
				record 3201, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间存在源地址全部冗余";
			} else {
#				pass $task_id, 3201, 'pass', "访问控制策略 $array[$j] 与 $array[$k] 之间不存在源地址全部冗余";
				record 3201, "TASK[$task_id] 访问控制策略 $array[$j] 与 $array[$k] 之间不存在源地址全部冗余";
			}
			$k++;
		}
		shift @array;
